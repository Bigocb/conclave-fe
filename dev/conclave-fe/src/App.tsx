...
const SidebarItem = ({ icon: Icon, label, active = false, onClick }: SidebarItemProps) => (
  <div 
    onClick={onClick} 
    className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-all duration-200 rounded-lg group ${active ? 'bg-aviation-accent/10 text-aviation-accent' : 'text-slate-500 hover:bg-white/5 hover:text-slate-300'}`}
  >
    <Icon size={18} className={`${active ? 'text-aviation-accent' : 'text-slate-500 group-hover:text-slate-300'} transition-colors`} />
...
export default function App() {
  ...
  const renderDashboard = () => (
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="col-span-1 lg:col-span-2 p-8 bg-aviation-panel border border-aviation-border rounded-2xl relative overflow-hidden group">
          <div className="absolute inset-0 bg-gradient-to-br from-aviation-accent/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            ...
            <div className="hidden md:block h-16 w-px bg-aviation-border" />
        ...
        <div className="p-8 bg-aviation-panel border border-aviation-border rounded-2xl">
          ...
          <div className="space-y-5">
            {[
              { label: 'API Latency', value: '14ms', color: 'text-aviation-accent' },
              { label: 'Pulse Stream', value: status, color: status === 'connected' ? 'text-aviation-accent' : 'text-red-400' },
  ...
  const renderView = () => {
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center gap-4 border border-dashed border-aviation-border rounded-2xl bg-black/20">
        <div className="w-48 h-1 bg-aviation-border overflow-hidden rounded-full">
  ...
  return (
    <div className="flex h-screen w-full bg-aviation-bg text-slate-200 overflow-hidden">
      {!isMobile && (
        <aside className="w-64 bg-aviation-panel border-r border-aviation-border flex flex-col shadow-2xl z-10">
          <div className="p-6 border-b border-aviation-border bg-black/30">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-aviation-accent rounded-lg flex items-center justify-center text-black font-black text-xs shadow-[0_0_10px_rgba(0,217,139,0.4)]">C</div>
          ...
          <div className="p-4 border-t border-aviation-border flex flex-col gap-4 bg-black/30">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-16 border-b border-aviation-border flex items-center justify-between px-8 bg-aviation-panel/80 backdrop-blur-md z-10">
          <div className="text-xs mono text-slate-500 uppercase tracking-[0.3em] font-bold">System // {activeView}</div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1 bg-black/60 rounded-full border border-aviation-border">
