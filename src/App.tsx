...
export default function App() {
  ...
  return (
    <div className="flex h-screen w-full bg-aviation-bg text-slate-200 overflow-hidden font-mono">
      <aside className="hidden md:flex w-64 bg-aviation-panel border-r border-aviation-border flex-col shadow-xl">
        ...
        <nav className="flex-1 py-4">
          {views.map(v => (
            <SidebarItem key={v.id} icon={v.icon} label={v.label} active={activeView === v.id} onClick={() => setActiveView(v.id)} />
          ))}
        </nav>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-aviation-panel border-t border-aviation-border flex items-center justify-around px-2 z-50">
        {views.map(v => {
          const Icon = v.icon;
          return (
            <button key={v.id} onClick={() => setActiveView(v.id)} className={`flex flex-col items-center justify-center gap-1 w-full h-full transition-colors ${activeView === v.id ? 'text-aviation-accent bg-aviation-accent/5' : 'text-slate-500'}`}>
              <Icon size={20} />
              <span className="text-[9px] uppercase font-bold">{v.label}</span>
            </button>
          );
        })}
      </nav>
        ...
        <header className="h-16 border-b border-aviation-border flex items-center justify-between px-4 md:px-8 bg-aviation-panel shadow-sm">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <div className="flex items-center gap-2 px-2 md:px-3 py-1 bg-black/40 rounded border border-aviation-border">
              ...
              <span className="text-[9px] md:text-[10px] mono text-aviation-accent uppercase font-bold">
                Pulse {status}
              </span>
            </div>
            <button onClick={logout} className="md:hidden p-2 text-slate-500 hover:text-red-400">
              <LogOut size={18} />
            </button>
          </div>
        </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="col-span-1 lg:col-span-2 h-auto lg:h-64 bg-aviation-panel border border-aviation-border rounded-lg p-6 md:p-8 noc-glow relative overflow-hidden shadow-inner">
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
                  <div className="text-4xl md:text-6xl font-black mono text-white tracking-tighter">
